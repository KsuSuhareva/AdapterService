...
public class RestTemplateJsonTest extends IntegrationTest {
    ...
    public void getFineWithValidDateForIndividual() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(post(FINE_GET_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SendRequest(null, "12AA123456", INDIVIDUAL))))
    ...
    public void getFineWithValidDateForLegalEntity() throws Exception {
        ...
        mockServer.expect(once(), requestTo(REQUEST_SAVE_URL))
                .andRespond(withStatus(HttpStatus.ACCEPTED)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(objectMapper.writeValueAsString(request.getUuid())));
        ...
        MvcResult mvcResult = mockMvc.perform(post(FINE_GET_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SendRequest(null, "1234567890", LEGAL_ENTITY))))
                .andExpect(status().is(HttpStatus.OK.value()))
                .andDo(print())
                .andReturn();
        Fine fine = objectMapper.readValue(
